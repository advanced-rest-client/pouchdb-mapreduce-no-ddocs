...
const stringMd5 = require('pouchdb-md5').stringMd5;
...
adapters.forEach(function(adapter) {
  ...
  testConfigs.forEach(function(config) {
    const suiteName = 'mapreduce.spec.js-' + adapter + '-' + config.viewType +
      '-auto=' + config.auto;
...
function tests(suiteName, dbName, dbType, viewType, auto) {
  describe(suiteName, function() {
        ...
        return db.bulkDocs({
          ...
          resp.rows[2].key.should.equal('key3');
            ...
            .then(function() {
              ...
              resp.rows.should.have
                  .length(4, 'startkey=key4, endkey=key1 descending without ' +
              'inclusive end');
    ...
    it('Test basic view collation', async () => {
      ...
      const res2 = await db.query(queryFun,
          { descending: true, reduce: false });
      res2.rows.forEach(function(x, i) {
    ...
    it('Query result should include _conflicts', function() {
      ...
      return testUtils.fin(db.info().then(function() {
        ...
        return remote.info().then(function() {
          const replicate = testUtils.promisify(
              db.replicate.from, db.replicate);
          ...
          return createView(db, {
            ...
            return db.bulkDocs({
              ...
              should.not.exist(res.rows[0].doc._doc_id_rev,
                  '_doc_id_rev is leaking but should not');
            });
          });
        });
    ...
    it.skip('Map documents on 0/null/undefined/empty string', function() {
      const db = new PouchDB(dbName);
      return createView(db, {
        map: function(doc) {
          emit(doc.num);
        ...
        return db.bulkDocs({ docs: docs }).then(function() {
          ...
          return Promise.all(emptyKeys.map(function(emptyKey) {
            return db.query(mapFunction,
                { key: emptyKey }).then(function(data) {
    ...
    it('Testing query with keys', function() {
      ...
      return createView(db, {
        ...
        return db.bulkDocs({
          ...
          data.rows.should.have.length(5, 'returns all docs');
          opts.keys = [];
          return db.query(queryFun, opts);
        }).then(function(data) {
          ...
          data.rows.should.have.length(5, 'returns 7 docs in correct order');
          ...
          data.rows[4].doc._id.should.equal('doc_empty');
    ...
    it('Testing query with multiple keys, multiple docs', function() {
      ...
      return createView(db, {
        ...
        return db.bulkDocs({
          docs: [
            { _id: '0', field1: 0, field2: '' },
            { _id: '1a', field1: 1, field2: '' },
            { _id: '1b', field1: 1, field2: '' },
            { _id: '1c', field1: 1, field2: '' },
    ...
    it.skip('should query correctly with undefined key/values', function() {
      const db = new PouchDB(dbName);
      const docs = {
        docs: [
          { _id: 'doc1' },
    ...
    it('should query correctly after replicating and other ddoc', function() {
      ...
      return createView(db, {
        ...
        return db.bulkDocs({ docs: [{ name: 'foobar' }] }).then(function() {
          ...
          const db2 = new PouchDB('local-other');
